...
template <typename T>
  struct WindowBuffer {
    public:
      auto streamPosition( size_t position ) -> size_t {};
      auto endPosition() -> size_t {};
      auto index( size_t index ) -> size_t {};
      auto mappedPosition( size_t position ) -> size_t {};

      WindowBuffer( size_t count );
    private:
      size_t head;
      size_t tail;
      size_t position;
